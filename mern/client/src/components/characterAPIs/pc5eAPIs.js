...
export const fetchClassSpellcasting = async (classIndex) => {
  try {
    const response = await axios.get(`${BASE_URL}/classes/${classIndex.toLowerCase()}/spellcasting`);
    return response.data;
  } catch (error) {
...
export const fetchSpellDetails = async (spellIndex) => {
  try {
    const response = await axios.get(`${BASE_URL}/spells/${spellIndex.toLowerCase()}`);
    return {
...
export const fetchSpellSlots = async (classIndex, level) => {
    ...
    // Fallback to API
    try {
      const response = await axios.get(`${BASE_URL}/classes/${classIndex.toLowerCase()}/levels/${level}`);
...
export const fetchClassFeatures = async (classIndex, level) => {
  try {
    ...
    for (let l = 1; l <= level; l++) {
      const response = await axios.get(`${BASE_URL}/classes/${classIndex.toLowerCase()}/levels/${l}`);
